...
// synchronization barrier for simultaneous run of all threads
...
{       
	if (sync_threads_barrier_initialized) {
		...
		if ((errno = pthread_barrier_destroy(&sync_threads)))
...
void *thread_func(void *arg)
{
	...
	printf("The thread %d (0x%lX) is waiting for the other threads: %s.%03ld\n", id, pthread_self(), timestr, tv.tv_usec/1000);

	// wait at the barrier
	rc = pthread_barrier_wait(&sync_threads);
	...
	switch (errno = rc) {	// check the return code
		case PTHREAD_BARRIER_SERIAL_THREAD:
			...
			printf("%d (0x%lX): All threads are resumed: %ld.%03ld\n", id, pthread_self(), tv.tv_sec, tv.tv_usec/1000);
	...
	localtime_r(&tv.tv_sec, &localtime);	// transform time to broken down format (for strftime)
	strftime(timestr, sizeof(timestr), TIME_FMT, &localtime);
	printf("The thread %d (0x%lX) is resumed: %s.%03ld\n", id, pthread_self(), timestr, tv.tv_usec/1000);
...
int main(int argc, char *argv[])
{
	...
	if ((errno = pthread_barrier_init(&sync_threads, NULL, THREADS))) {
